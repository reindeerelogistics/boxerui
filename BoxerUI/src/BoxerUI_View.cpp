...
void BoxerUI_View::indexwindow(bool *boxer_analytics)
...
bool BoxerUI_View::sideNav()
{
...

	{
		...
		if (button(ICON_MD_SETTINGS "##settings"))
		{
...

	{
		...
		if (button("%s##Camera" ICON_MD_CAMERA))
		{
...

	{
		//Analytics
		ImGui::PushFont(ImGui::GetFont()->ContainerAtlas->Fonts[0]);
		if (button(ICON_MD_CAMERA_ENHANCE "##stream"))
		{
	...
	case NavItem::Home:
		/* Go to home... */
		std::cout << "Went to Home" << std::endl;
		break;
	...
	case NavItem::Stream:
		...
		std::cout << "Went to Stream" << std::endl;
		break;
